...
def evaluate_config(label, n_tables, n_bits, n_clusters, n_probe_clusters, n_probes):
...
# Target 85% and 95% configs to test single-threaded execution
configs = [
    ("85% Target", 9, 7, 256, 6, 6),
    ("95% Target", 20, 10, 256, 16, 10),
]

print(f"\n--- Verifying Single-Threaded Configurations on Full Dataset ---")
print(f"{'Target':<12} | {'tables':<6} | {'bits':<4} | {'clusters':<10} | {'probe_cl':<8} | {'probes':<6} | {'Recall@5':<10} | {'Query (ms)':<12} | {'Fit (s)':<10} | {'Avg Dists':<10}")
print("-" * 115)

for label, t, b, c, pc, p in configs:
    fit_t, q_t, recall, avg_dists = evaluate_config(label, t, b, c, pc, p)
    print(f"{label:<12} | {t:<6} | {b:<4} | {c:<10} | {pc:<8} | {p:<6} | {recall:<10.4f} | {q_t:<12.4f} | {fit_t:<10.4f} | {avg_dists:<10.1f}")
