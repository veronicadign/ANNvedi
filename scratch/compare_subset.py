...
# Use 100k subset for fast comparative benchmarking
subset_size = 100000
train_sub = train[:subset_size]
test_sub = test[:100] # evaluate on 100 queries
print(f"Subset shapes: Train={train_sub.shape}, Test={test_sub.shape}")

# Precompute linear brute force ground truth
print("\nComputing linear ground truth...")
...
# Add linear baseline to results if <= 5.0 ms
if query_time_linear <= 5.0:
    results.append({
        "variant": "Linear (Brute Force)",
        "scenario": "baseline",
        "recall": 1.0000,
        "query_time": query_time_linear,
        "fit_time": fit_time_linear,
        "dist_count": len(train_sub)
    })
...
for var in variants:
    ...
    for scenario_name, scenario_cfg in scenarios_data.items():
        ...
        # Query index
        algo.get_n_distances() # reset/dummy read
        t0 = time.time()
        preds = []
        for q in test_sub:
        ...
        if query_time <= 5.0:
            ...
            print(f"    -> Skipped (Query Time {query_time:.2f} ms > 5 ms)")

# Print results as Markdown table
print("\n### Unified Performance Comparison Table (100k Subset)")
