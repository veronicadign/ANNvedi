...
# Target configs around 95% recall
configs = [
    (20, 10, 256, 16, 10),
    (20, 10, 256, 18, 10),
    (20, 10, 256, 16, 12),
...
print(f"\n--- Testing Configurations targeting >= 95% Recall ---")
...
for n_tables, n_bits, n_clusters, n_probe_clusters, n_probes in configs:
    fit_t, q_t, recall, avg_dists = evaluate_config(n_tables, n_bits, n_clusters, n_probe_clusters, n_probes)
    print(f"{n_tables:<10} | {n_bits:<8} | {n_clusters:<10} | {n_probe_clusters:<10} | {n_probes:<8} | {recall:<10.4f} | {q_t:<12.4f} | {fit_t:<10.4f} | {avg_dists:<10.1f}")
