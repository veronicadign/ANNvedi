...
inline float compute_l2_distance_quantized(const uint8_t* a, const float* q_shifted, size_t dim, const float* scale, float threshold = std::numeric_limits<float>::max()) {
#if defined(HAS_AVX512)
    __m512 sum_vec = _mm512_setzero_ps();
    size_t i = 0;
